...
#define LV_PRIVATE_HANDLE(T) \
    struct T##_t;            \
    typedef struct T##_t** T
#define LV_PRIVATE_POINTER(T) \
    struct T##_t;             \
    typedef struct T##_t* T
...
enum
{
    ...
    mgDupeEltInSet, // Operation attempted to create a set from malformed data that contained duplicates.
    mgDupeKeyInMap, // Operation attempted to create a map from malformed data that contained duplicates.
    mgErrSentinel, // 132
...
#if __cplusplus
inline uChar* PStrBuf(const PStr b)
{
    return &(b[1]);
}
inline uChar& PStrLen(const PStr b)
{
    return b[0];
} // # of chars in string
inline const uChar* PStrBuf(const ConstPStr b)
{
    return &(b[1]);
}
inline uChar PStrLen(const ConstPStr b)
{
    return b[0];
} // # of chars in string
inline uInt16 PStrSize(const ConstPStr b)
{
    return static_cast<uInt16>(b ? PStrLen(b) + 1 : 0);
} // # of bytes including length;
...
#define LHStrPtr(sh) ((sh) ? *(sh) : nullptr) // Get LStr* (produces null ptr for null handle
#define LHStrBuf(sh) ((sh) ? (&(*(sh))->str[0]) : nullptr) // same as LStrBuf, but on string handle
...
enum
{
    kDSHandleZoneCheckNone,
    kDSHandleZoneCheckResize,
    kDSHandleZoneCheckAll
};
...
typedef struct
{
    ...
    uInt64 maxFreeSize; // largest free contiguous block in bytes (obsolete)
...
#if Compiler == kGCC // protect IntFrom4Chars usages from GCC warnings
...
typedef enum _FMFileType
{
    ...
    kUnknownFileType = IntFrom4Chars('\?\?\?\?'), ///< unknown file type
    kTextFileType = IntFrom4Chars('TEXT'), ///< text file
    kHardDiskDirType = IntFrom4Chars('hdsk'), ///< hard disk directory
    kFloppyDirType = IntFrom4Chars('flpy'), ///< floppy drive type
    kNetDriveDirType = IntFrom4Chars('srvr') ///< network volume
} FMFileType;
...
typedef enum _FMFileCreator
{
    ...
    kUnknownCreator = IntFrom4Chars('\?\?\?\?'), ///< unknown creator
    kLVCreatorType = IntFrom4Chars('LBVW') ///< LabVIEW creator
} FMFileCreator;

#if Compiler == kGCC && (__GNUC__ > 4 || (__GNUC__ == 4 && __GNUC_MINOR__ >= 6))
